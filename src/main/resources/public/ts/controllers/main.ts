...
export let main = ng.controller("EdtController", [
  ...
  function (
    ...
    $location,
    $timeout,
    courseTagService: ICourseTagService,
    ...
    const WORKFLOW_RIGHTS =
      Behaviours.applicationsBehaviours.edt.rights.workflow;
    ...
    $scope.syncStructure = async (structure: Structure): Promise<void> => {
      ...
      promises.push(
        $scope.structure.sync(model.me.type === USER_TYPES.teacher)
      );
      promises.push(initTimeSlots());
      await Promise.all(promises);
      $scope.courseTags = await courseTagService.getCourseTags(
        $scope.structure.id
      );
      changeDatesOnSunday();
      switch (model.me.type) {
        ...
        case USER_TYPES.relative: {
          if ($scope.structure.students.all.length > 0) {
            if (!$scope.children) {
              ...
              $scope.children.sort((c, cc) => {
                if (
                  ...
                  (c.lastName === cc.lastName && c.firstName < cc.firstName)
                ) {
                  return -1;
                } else {
                  return 1;
            ...
            $scope.params.group = _.map(
              structure.students.all[0].classes,
              (groupid) => {
    ...
    $scope.switchChild = async (child: Student): Promise<void> => {
      $scope.child = child;
      $scope.structure =
        child.structures.length > 0
          ? new Structure(child.structures[0].id, child.structures[0].name)
          : null;
    ...
    $scope.switchStructure = async (structure: Structure): Promise<void> => {
      ...
      if ($scope.structure == null) {
        $scope.structure = new Structure(
          lang.translate("all.structures.id"),
          lang.translate("all.structures.label")
        );
      }
      ...
      await PreferencesUtils.updateStructure({
        id: $scope.structure.id,
        name: $scope.structure.name,
      });
    ...
    $scope.canManage = (): boolean =>
      model.me.hasWorkflow(WORKFLOW_RIGHTS.manage);
    ...
    $scope.getStudentGroup = (user: Student): Group =>
      _.findWhere($scope.structure.groups.all, { externalId: user.classes[0] });

    function filterCourses(): void {
      $scope.structure.calendarItems.all.map(
        (item: CalendarItem, i: number) => {
          if (
            (item && moment(item.endCourse).isBefore(item.endDate)) ||
            (item && !item.startMoment)
          ) {
    ...
    $scope.syncCourses = async (): Promise<void> => {
      ...
      if ($scope.isRelative() && $scope.child) {
        await $scope.structure.calendarItems.getGroups(
          $scope.structure.groups.all,
          null,
          $scope.child.id
        );
      ...
      if (!isUpdateData && $scope.isRelative()) {
        ...
        $scope.params.group = $scope.structure.groups.all.filter(
          (item: Group) => arrayIds.indexOf(item.id) > -1
        );
      }
      ...
      if (
        model.me.type === USER_TYPES.personnel ||
        model.me.type === USER_TYPES.teacher
      )
        $scope.params.group.map((g) => {
          ...
          $scope.params.deletedGroups.classes.map((c) => {
            $scope.params.deletedGroups.groupsDeleted.map(
              (gg, index: number) => {
                if (gg && c && gg.id === c.id)
        ...
        for (let i = 0; i < $scope.params.group.length; i++) {
          ...
          if (
            ...
            (group.color === "" ||
              ...
              $scope.structure.groups.all.indexOf(group) === -1)
          ) {
            $scope.params.group[i] = $scope.structure.groups.all.filter(
              (res) => group.name == res.name
            )[0];
          }
        }
        ...
        $scope.params.deletedGroups.groupsDeleted.map((g: Group) => {
          $scope.params.group.map((gg) => {
            if (
              ...
              gg &&
              g.id === gg.id &&
              gg.type_groupe !== Utils.getClassGroupTypeMap()["MANUAL_GROUP"]
            ) {
      ...
      await $scope.structure.calendarItems.sync(
        $scope.structure,
        $scope.params.user,
        $scope.params.group,
        $scope.structures,
        $scope.isAllStructure
      );
    ...
    $scope.dropGroup = (group: Group): void => {
      if (
        group &&
        (group.type_groupe != 0 || group.type_groupe === undefined) &&
        group.color != ""
      )
    ...
    $scope.getTeacherFromId = (id: string): Teacher => {
      return $scope.structure.teachers.all.find(
    ...
    template.open(
      "deleteOccurrencePagePopup",
      "main/occurrence-or-course-delete-popup"
    );
     ...
     */
    $scope.chooseTypeEdit = async (
      itemId: string,
      start?: string,
      end?: string,
      isDrag?: boolean
    ): Promise<void> => {
      $scope.courseToEdit = _.findWhere(
        _.pluck($scope.structure.calendarItems.all, "course"),
        { _id: itemId }
      );
      $scope.paramEdition = {
        start: start,
      ...
      $scope.occurrenceDate = $scope.courseToEdit.getNextOccurrenceDate(
        Utils.getFirstCalendarDay()
      );
      ...
      if ($scope.isAbleToChooseEditionType($scope.courseToEdit, start)) {
        if ($scope.courseToEdit.recurrenceObject === undefined) {
          ...
          try {
            recurrence = await courseService.getCourseRecurrenceDates(
              $scope.courseToEdit.recurrence
            );
          } catch (e) {
            recurrence = { startDate: "", endDate: "" };
          }
          $scope.courseToEdit.recurrenceObject = {
            start: DateUtils.getDateFormat(recurrence.startDate),
        ...
        $scope.calendarUpdateItem(
          itemId,
          $scope.paramEdition.start,
          $scope.paramEdition.end
        );
      }
      Utils.safeApply($scope);
    };
     ...
     */
    $scope.isAbleToChooseEditionType = (
      course: Course,
      start: Moment
    ): boolean => {
      ...
      let atLeastOnePreviousOccurence: boolean = moment(
        previousOccurrence
      ).isAfter(moment(start));
      let upcomingOccurrence: string = course.getNextOccurrenceDate(start);
      let atLeastOneOccurence: boolean = moment(
        course.getNextOccurrenceDate(upcomingOccurrence)
      ).isBefore(moment(start));

      let newDay: string = moment(start).format("DD");
      let previousDay: string = moment(course.startDate).format("DD");
      return (
        course.recurrence !== undefined &&
        ((atLeastOneOccurence && moment(upcomingOccurrence).isAfter(now)) ||
          (moment(previousOccurrence).isAfter(now) &&
            atLeastOnePreviousOccurence) ||
          (newDay != previousDay &&
            moment(course.getNextOccurrenceDate(upcomingOccurrence)).isAfter(
              start
            )) ||
    ...
    $scope.getFirstRecurrenceDate = (course: Course): string => {
      if (course && course.recurrenceObject) {
        return $scope.getSimpleFRDateFormat(
          moment(course.recurrenceObject.start).isBefore(moment())
            ? moment()
            : course.recurrenceObject.start
        );
    ...
    $scope.deleteCourses = async (): Promise<void> => {
      const promises = [];
      $scope.params.coursesToUpdate.map((course: Course) =>
        promises.push(course.delete(course._id))
      );
      await Promise.all(promises);
      $scope.syncCourses();
    };
    ...
    $scope.updateLabels = async (): Promise<void> => {
      let courseIds: Array<string> = $scope.params.coursesToUpdate.map(
        (course: Course) => course._id
      );
      await courseService.updateCoursesTag(
        courseIds,
        $scope.updateMultipleLightboxForm.tagId
      );
      $scope.syncCourses();
    };
    ...
    $scope.toggleSelectAllCourses = (): void => {
      ...
      DragAndDrop.togglePrepareDeleteAllCourses(
        $scope,
        $scope.isAllCoursesSelected
      );
    };
     ...
     */
    $scope.deleteCourseOccurrences = async (
    ...
    $scope.toogleFilter = async (filter: Group): Promise<void> => {
      ...
      if (!$scope.isFilterActive(filter)) {
        ...
        $scope.params.deletedGroups.groupsDeleted.forEach(
          (group, index: number) => {
        ...
        groups.forEach((group) => {
          groups.push(
            $scope.structure.groups.all.filter(
              (res) => group.name == res.name
            )[0]
          );
        });
        groups.forEach((group) => $scope.dropGroup(group));
      }
    ...
    $scope.initDateCreatCourse = (param?: any, course?: Course): void => {
      if (
        model.calendar.newItem ||
        (param && param["beginning"] && param["end"])
      ) {
        let timeSlotInfo: { beginning: string; end: string } = {
          beginning: param
            ? param.beginning
            : model.calendar.newItem.beginning.format("x"),
          end: param ? param.end : model.calendar.newItem.end.format("x"),
        };

        let startTime: Moment = moment
          .utc(timeSlotInfo["beginning"], "x")
          .add("hours", -moment().format("Z").split(":")[0])
          .seconds(0)
          .millisecond(0);
        let endTime: Moment = moment
          .utc(timeSlotInfo["end"], "x")
          .add("hours", -moment().format("Z").split(":")[0])
          .seconds(0)
          .millisecond(0);
        ...
        } else $scope.courseOccurrenceForm = new CourseOccurrence();
        return moment();
      }
    };
    ...
    this.$onInit = (): void => {
      ...
      $scope.onEventUpdateStructure
        .asObservable()
        .subscribe(async (structure: Structure) => {
          await $scope.switchStructure(
            new Structure(structure.id, structure.name)
          );
          await $scope.syncCourses();
        });
    };
    ...
    function getStructure(): Structure {
      if (window.preferenceStructure && window.preferenceStructure.id) {
        if (
          window.preferenceStructure.id == lang.translate("all.structures.id")
        ) {
          $scope.isAllStructure = true;
          return new Structure(
            window.preferenceStructure.id,
            window.preferenceStructure.name
          );
        }
        let structure: Structure = $scope.structures.all.find(
          (structure: Structure) =>
    ...
    route({
      ...
      edit: async (params: any): Promise<void> => {
        $scope.course = $scope.structure.calendarItems.all.find(
          (course) => course._id === params.idCourse
        );
        if (!$scope.course) {
          $scope.goTo("/");
          return;
